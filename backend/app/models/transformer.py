...
class TransformerClassifier(pl.LightningModule):
    """Graph Transformer backbone (``torch_geometric.nn.TransformerConv``).

    Multi-head attention message passing, structurally parallel to the GAT
    module: each conv emits ``hidden_dim // num_heads`` channels per head and
    concatenates back to ``hidden_dim``. Unlike GATConv, TransformerConv adds no
    implicit self-loops, so it also lifts cleanly under ``to_hetero`` (see
    ``app.models.hetero_wrapper``).
    """
    ...
    def __init__(
        ...
        # First layer
        self.convs.append(TransformerConv(num_features, hidden_dim // num_heads, heads=num_heads, concat=True))
        self.bns.append(BatchNorm1d(hidden_dim))

        # Hidden layers
        for _ in range(num_layers - 1):
            self.convs.append(TransformerConv(hidden_dim, hidden_dim // num_heads, heads=num_heads, concat=True))
            self.bns.append(BatchNorm1d(hidden_dim))
